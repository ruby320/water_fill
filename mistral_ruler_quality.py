...
DEFAULT_OUTPUT = Path("results/ruler_16k_all_methods_r20_v1")
DEFAULT_TASKS = ("qa_1", "niah_multivalue", "niah_multikey_3", "cwe")
METHODS = (
    "fullkv", "formula_waterfill_r20", "global_empirical_topk_r20", "ada_snapkv_r20",
    "pyramidkv_r20", "streamingllm_r20", "snapkv_r20",
)
SCHEMA_VERSION = 1
...
def summarize(rows: list[dict[str, Any]], args: Any, metric: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "methods": list(args.methods), "target_prompt_kv_ratio": args.target_kv_ratio,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="RULER quality comparison for Water-fill and KV baselines")
...
def main() -> None:
    ...
    for ordinal, record in enumerate(records, 1):
        methods_for_record = pending_methods(record["source_id"], completed, tuple(args.methods))
        ...
        try:
            ...
            score_configs = {}
            if set(methods_for_record) & {"ada_snapkv_r20", "snapkv_r20"}:
                score_configs["adakv"] = (32, 7, "max")
            if "pyramidkv_r20" in methods_for_record:
            ...
            if "fullkv" in methods_for_record:
                variants["fullkv"] = (None, None, None)
            if "formula_waterfill_r20" in methods_for_record:
                ...
                variants["formula_waterfill_r20"] = (mapping, None, details)
            if "global_empirical_topk_r20" in methods_for_record:
                mapping, details = global_empirical_topk_block_map(
                    32, record["prompt_tokens"], args.sink_tokens + args.local_tokens,
                    args.block_size, args.target_kv_ratio, masses,
                )
                variants["global_empirical_topk_r20"] = (mapping, None, details)
            if "snapkv_r20" in methods_for_record:
                mapping, details = snapkv_token_map(
                    patch.prompt_token_scores["adakv"], args.target_kv_ratio,
                )
                variants["snapkv_r20"] = (None, mapping, details)
            if "ada_snapkv_r20" in methods_for_record:
                mapping, details = adakv_token_map(
                    patch.prompt_token_scores["adakv"], args.target_kv_ratio, 0.2,
                )
                variants["ada_snapkv_r20"] = (None, mapping, details)
            if "pyramidkv_r20" in methods_for_record:
                mapping, details = pyramidkv_token_map(
                    patch.prompt_token_scores["pyramidkv"], args.target_kv_ratio, 20, 64,
                )
                variants["pyramidkv_r20"] = (None, mapping, details)
            if "streamingllm_r20" in methods_for_record:
                mapping, details = streamingllm_token_map(
                    32, int(config.num_key_value_heads), record["prompt_tokens"],
                    args.target_kv_ratio, args.sink_tokens,
                )
                variants["streamingllm_r20"] = (None, mapping, details)
            del output, prompt
