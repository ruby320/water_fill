...
if [[ "${SMOKE}" == "1" ]]; then
  OUTPUT_DIR="${OUTPUT_DIR:-results/gov_report_task_quality_smoke}"
  LOG_FILE="${LOG_FILE:-logs/gov_report_task_quality_smoke.log}"
else
  OUTPUT_DIR="${OUTPUT_DIR:-results/gov_report_task_quality}"
  LOG_FILE="${LOG_FILE:-logs/gov_report_task_quality.log}"
fi
mkdir -p "${OUTPUT_DIR}" logs
COMMAND=(
  "${PYTHON_BIN}" mistral_task_quality.py
  --model_path "${MODEL_PATH}" --longbench_root "${LONGBENCH_ROOT}" --output_dir "${OUTPUT_DIR}"
  --num_samples 5 --seed 42 --max_context_length 32768 --max_new_tokens 512
