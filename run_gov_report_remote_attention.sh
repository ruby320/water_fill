...
if [[ "${SMOKE}" == "1" ]]; then
  OUTPUT_DIR="${OUTPUT_DIR:-results/gov_report_remote_attention_smoke}"
  LOG_FILE="${LOG_FILE:-logs/gov_report_remote_attention_smoke.log}"
else
  OUTPUT_DIR="${OUTPUT_DIR:-results/gov_report_remote_attention}"
  LOG_FILE="${LOG_FILE:-logs/gov_report_remote_attention.log}"
fi
mkdir -p "${OUTPUT_DIR}" logs
COMMAND=(
  "${PYTHON_BIN}" mistral_remote_attention.py
  ...
  --b_blocks 8 --random_repeats 5 --block_size 128
