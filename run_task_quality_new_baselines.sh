...
RUN_TAG="${RUN_TAG:-new_baselines_r20_v1}"
TASKS="${TASKS:-gov_report hotpotqa lcc multi_news}"
...
for TASK in ${TASKS}; do
  ...
  COMMAND=(
    ...
    --min_prompt_tokens 2580 --block_size 128 --sink_tokens 4 --local_tokens 512
    --formula_target_kv_ratio 0.20 --formula_target_kv_ratios 0.20
    --methods streamingllm_r20 snapkv_r20 --dtype float16
    --device_map auto --attn_implementation sdpa --max_memory "${MEMORY_LIMITS[@]}" --resume
  )
